 ...
 * 方便子节点查找
 */
cc.Node.prototype.find2 = function(path,typeName){
    if(typeName){
        return cc.find(path,this).getComponent(typeName);
    }
    return cc.find(path,this);
}
...
//设置文本
cc.setText = function(node,text){ 
    if(!node || node.isValid == false) {
        return console.error("node is null");
    }
    
    if(node.hasComponent(cc.Label)){
        (text  || typeof(text) == "number") && (node.getComponent(cc.Label).string = text);
    }else if(node.hasComponent(cc.Button)){
        (text  || typeof(text) == "number")  && (node.getComponent(cc.Button).string = text);
    }else if( node.hasComponent(cc.ProgressBar)){ 
        (text  || typeof(text) == "number")  && (node.getComponent(cc.ProgressBar).progress = text); 
    }else if( node.hasComponent(cc.Sprite)){ 
        (text  || typeof(text) == "number")  && (node.getComponent(cc.Sprite).spriteFrame = text); 
    } 
}
...
// cc.Btn = function(btn,isClick){ 
//     if(btn.hasComponent(cc.Button)){
//         btn.getComponent(cc.Button).interactable = isClick;
//     } 
// }
