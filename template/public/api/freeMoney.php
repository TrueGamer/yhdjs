...
// 是否超过最大次数限制
if (roleManager_freeMoneyNumOverOnLimit($player)){
	return RstMsg(error_getCommonError('OVER_ON_FREE_MONEY_LIMIT'), $msgID);
};
...
// 先添加次数
$num = roleManager_addFreeMoneyNum($player);

// 是否在冷却
if (roleManager_freeMoneyNumIsCooling($player, $num)){
	return RstMsg(error_getCommonError('FREE_MONEY_COOLING'), $msgID);
};
...
// 派发免费阳光奖励
$addMoney = roleManager_freeMoneyReward($player, $num, $awardBase, $rewardList);

// 保存本次领取免费阳光的时间
roleManager_setFreeMoneyTime($player);
