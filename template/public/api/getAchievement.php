...
// 效验成就是否重复完成
if(!roleManager_repeatCompleteAchievement($player, $id)){
    return RstMsg(error_getCommonError('REPEAT_GET_ACHIEVEMENT'), $msgID);
};

// 效验成就是否达到要求
if(!roleManager_achievementCompleteAsk($player, $id)){
    return RstMsg(error_getCommonError('ACHIEVEMENT_NOT_COMPLETE'), $msgID);
};
...
// 领取成就任务的奖励,根据倍数领取奖励
for ( $i = 0; $i< $multi; $i++ ){
    roleManager_getAchievementReward($player, $id, $connObj, $rewardList);
}

// 重新设置成就任务
roleManager_setAchievementRecord($player, $id);
