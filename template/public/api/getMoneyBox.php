...
$msgID = $MESSAGE['GET_MONEY_BOX'];
$uid  = number_format($parameter['uid'],0, '', ''); // uid
$rate = (int)$parameter['rate'];     // 领取的倍率
if ( empty($rate) || $rate > 3 ){
    $rate = 1;
}
...
// 校验钻石是否足够
$moneyBoxExpend = roleManager_getMoneyBoxExpend($player);
$goodsType = getSysConfig('GOODS_TYPE');
$itemIds = getSysConfig('ITEM_ID');
// 判断角色钻石是否充足
if(!roleManager_goodsSufficient($player, $itemIds['DIAMOND'],$goodsType['DIAMOND'], $moneyBoxExpend )){
...
$money = roleManager_getMoneyBox($player);

if (!empty($money)){
    $goodsType = getSysConfig('GOODS_TYPE');
    $itemIds = getSysConfig('ITEM_ID');
    $realMoeny = $money*$rate;
    // 增加金币
    roleManager_addGoods($player, $itemIds['MONEY'],$goodsType['MONEY'], $realMoeny, $rewardList);
    // 清空当前存钱罐
    roleManager_clearMoneyBox($player);
	if ( $rate == 1 ){
		// 扣除存钱罐的钻石消耗
		roleManager_expendGoods($player, $itemIds['DIAMOND'],$goodsType['DIAMOND'], $moneyBoxExpend);
	}
}
