...
// 工作是否开始
if(roleManager_workNotStart($player)){
    return RstMsg(error_getCommonError('WORK_NOT_START'), $msgID);
};

// 判断工作是否完成
if(!roleManager_workAchieve($player)){
    return RstMsg(error_getCommonError('WORK_NOT_ACHIEVE'), $msgID);
};
...
// 发放工作奖励
for( $i = 0; $i < $multi; $i++ ){
    roleManager_getWorkReward($player, $awardBase,$rewardList);
}

// 清理工作数据
roleManager_clearWorkData($player);
