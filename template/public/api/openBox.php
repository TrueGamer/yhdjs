...
$msgID = $MESSAGE['OPEN_BOX'];
$uid  = number_format($parameter['uid'],0, '', ''); // uid
$site = $parameter['site'];     // 开启宝箱的位置
...
// 判断该位置上是否有宝箱
if( !petManager_isBoxOfSite($player, $site) ){
	return RstMsg(error_getCommonError('ONLY_OPEN_BOX'), $msgID);
}
...
$admark = @$parameter['syncAD']?(int)$parameter['syncAD'] : 0;

// 开启宝箱
petManager_openBox($player, $site, $boxUpdate, $admark, $rewardList);
