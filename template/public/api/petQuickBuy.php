...
// 计算当前最实惠的宠物
$result = petManager_getCostPerformanceHigPet($player);
$level = $result->level;
// 增加宠物的购买记录(方便后续计算)
petManager_addMoneyBuyRecordNum($player, $level);

// 获得需要的金钱花费
$price =  petManager_calculatePetMoneyCostByLevel($player, $level, 0);

//// 测试用，将快速购买宠物的金币消耗设置成0
//$price = 0;
...
// 判断角色金钱是否充足
if(!roleManager_goodsSufficient($player, $itemIds['MONEY'],$goodsType['MONEY'], $price )){
    return RstMsg(error_getCommonError('LACK_OF_MONEY'), $msgID);
}
...
// 扣除金币
roleManager_expendGoods($player, $itemIds['MONEY'],$goodsType['MONEY'], $price);
