...
$msgID = $MESSAGE['PET_SELL'];
$uid  = number_format($parameter['uid'],0, '', ''); // uid
$site = $parameter['site'];     // 出售宠物的位置
...
if( $siteData->type != $type["PET"]  ){
	return RstMsg(error_getCommonError('ONLY_SELL_PET'), $msgID);
}
...
// 计算出售宠物的价格
$price = petManager_calculatePetSellPrice($player, $site);
...
// 增加金币
roleManager_addGoods($player, $itemIds['MONEY'],$goodsType['MONEY'], $price, $rewardList);
