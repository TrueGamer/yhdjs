...
$msgID = $MESSAGE['PET_UPGRADE_REWARD'];
$uid  = number_format($parameter['uid'],0, '', ''); // uid
$multi  = @$parameter['multi']? $parameter['multi']:1; // 是否进行多倍领取
...
if (!petManager_allowGetPetUpgradeReward($player)){
    return RstMsg(error_getCommonError('NOT_PET_UPGRADE_AWARD'), $msgID);
}
...
petManager_handOutPetUpgradeReward($player, $multi, $rewardList);

petManager_clearMaxUpgradeRewardData($player);
