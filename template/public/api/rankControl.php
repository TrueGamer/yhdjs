...
$msgID = $MESSAGE['RANK_CONTROL'];
$uid  = number_format($parameter['uid'],0, '', ''); // uid
$rank = (int)$parameter['rank'];     // 段位值
...
// 设置当前关卡
roleManager_rankControl($player, $rank);

// 设置最大关卡
$maxRank = roleManager_setMaxRank($player, $rank-1);
if ( $maxRank ){
    //  非0，表示为第一次通过，发放通关奖励
    roleManager_grantUpAward($player, $maxRank, 1, $rewardList);
    roleManager_setUpAwardSign($player, $maxRank);
}
...
$messageRetData->isFirst = $maxRank;
