...
$msgID = $MESSAGE['SET_WORK'];
$uid  = number_format($parameter['uid'],0, '', ''); // uid
$id = $parameter['id'];     // 工作ID
if ( !isset($id) ){
...
// 工作是否有效
if (!roleManager_workValid($player, $id)){
    return RstMsg(error_getCommonError('WORK_NOT_VALID'), $msgID);
}

// 工作正在进行当中
if(!roleManager_workNotStart($player)){
    return RstMsg(error_getCommonError('WORKING'), $msgID);
};
...
// 设置工作
roleManager_setWorkData($player, $id);
